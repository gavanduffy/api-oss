...
class ApipieProviderConfig:
    ...
    base_url: str = 'YOUR_APIPPE_API_BASE_URL_HERE' # e.g., 'https://api.apipie.com/v1'
...
class APIClient:
    ...
    async def make_request(
        ...
        # **IMPORTANT**: Adjust headers for Apipie API authentication.
        # This example assumes a Bearer token.
        headers = {
            'Authorization': f'Bearer {sub_provider["api_key"]}',
            'Content-Type': 'application/json'
        }
        url = f'{self.config.base_url}/{endpoint}' # Adjust endpoint path as needed for Apipie
...
class EndpointHandler:
    ...
    async def handle_moderations(
        ...
        response = await self.api_client.make_request(
            endpoint='moderations', # Adjust endpoint if different for Apipie
            method='POST',
            sub_provider=sub_provider,
            data=payload,
            long_timeout=True
        ...
        model_instance = Model.get_model(model)
        request.state.user['credits'] -= model_inst
